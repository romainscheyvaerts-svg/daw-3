        ...
        <div className="flex space-x-1 shrink-0">
          ...
          {(isAudio || isMidiOrSampler) && (
              <button 
                ...
                className={`w-7 h-7 rounded-md flex items-center justify-center transition-all ${track.isTrackArmed ? 'bg-red-600 text-white animate-pulse shadow-[0_0_10px_red]' : 'bg-white/5 text-slate-600 hover:text-white'}`}
                title={track.isTrackArmed ? "Disarm Track" : "Arm Track for Recording"}
              >
                <div className={`w-2.5 h-2.5 rounded-full ${track.isTrackArmed ? 'bg-white' : 'bg-red-600'}`}></div>
              </button>
          )}
        </div>
      </div>
